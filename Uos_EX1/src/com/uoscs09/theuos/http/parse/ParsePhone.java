...
public class ParsePhone extends JerichoParse<PhoneItem> {
	...
	protected List<PhoneItem> parseHttpBody(Source source)
		...
		switch (howTo) {
		case ParseFactory.Value.BOTTOM: // 기타 주요시설 파싱(페이지 아랫부분의 전화번호 파싱)
		{
			...
			break;
		case ParseFactory.Value.BODY: // 각 편의시설 전화번호 파싱
		{
			...
			break;
		case ParseFactory.Value.SUBJECT: // 각 학과/학부 사무실 전화번호 파싱
		{
