    ...
    protected ArrayList<RestItem> parseHttpBody(Source source) throws IOException {
        ...
        for (int i = 0; i < restList.size(); i++) {
            ...
            breakfast = StringUtil.remove(
                    StringUtil.replaceHtmlCode(menuList.get(6 * i + 1).getFirstElement(HTMLElementName.PRE).getContent().toString()),
                    AMP)
                    .trim();

            lunch = StringUtil.remove(StringUtil.replaceHtmlCode(
                    menuList.get(6 * i + 3).getFirstElement(HTMLElementName.PRE).getContent().toString()
            ), AMP).trim();
